...
SUCURSALES = [
    ...
    {"id": 7, "nombre": "Casona Galerías", "alias": ["casona galerias"]},
...
def fecha_ayer_zetus():
    return datetime.now(TZ_MTY).strftime("%Y/%m/%d")

def consultar_zetus_linda_vista():
    payload = {
        "aut": {
            "id": ZETUS_ID,
            "tkn": ZETUS_TOKEN,
        },
        "dob": fecha_ayer_zetus(),
        "id_suc": 4  # 👈 GRLV
    }

    response = requests.post(ZETUS_API_URL, json=payload, timeout=30)
    response.raise_for_status()
    data = response.json()

    if not data.get("ok"):
        raise Exception("Zetus respondió error")

    return data

def extraer_total_zetus(data):
    total = 0.0

    for ticket in data.get("contenido", []):
        for pago in ticket.get("pagos", []):
...
def fetch_zetus_linda_vista():
    try:
        fecha_zetus = AYER.replace("-", "/")
        data = consultar_api_ventas_por_sucursal(fecha_zetus, 4)
        pagos = extraer_pagos_api(data, "GRLV", 4)

        total = round(sum(p["monto_pago"] for p in pagos), 2)

        return [
            {
                "sucursal": "Generales Linda Vista",
                "fecha": AYER,
                "total": total
            }
        ]
    except Exception as e:
        print("ERROR ZETUS:", e, flush=True)
        return []
...
def api_comparar_linda_vista():
    ...
    cortes_zetus = fetch_zetus_linda_vista()
