...
def subir_corte():
    if request.method == "POST":
        ...
        try:
            print("1. entrando a leer form y files", flush=True)
            form = request.form
            files = request.files
            print("2. form y files leídos", flush=True)
            ...
            print(f"3. datos base: sucursal={sucursal}, fecha={fecha}, turno={turno}, cajera={cajera}, total_corte_txt={total_corte_txt}", flush=True)
            ...
            print("4. total_corte convertido", flush=True)

            img1_file = files.get("imagen_1")
            img2_file = files.get("imagen_2")

            print(
                f"5. archivos detectados: "
                f"img1={img1_file.filename if img1_file and img1_file.filename else 'NO'} "
                f"| img2={img2_file.filename if img2_file and img2_file.filename else 'NO'}",
                flush=True
            )
            ...
            print("6. guardando imagen 1", flush=True)
            img1 = save_uploaded_file(img1_file, sucursal, fecha, turno, 1)
            print(f"7. imagen 1 guardada: {img1}", flush=True)

            print("8. guardando imagen 2", flush=True)
            img2 = save_uploaded_file(img2_file, sucursal, fecha, turno, 2)
            print(f"9. imagen 2 guardada: {img2}", flush=True)

            print("10. insertando en base de datos", flush=True)
            db = get_db()
            db.execute(
                """
            ...
            db.commit()
            print("11. insert completado", flush=True)
